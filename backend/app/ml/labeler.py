...
class OutcomeLabeler:
    """Create forward-looking labels without using future data as features."""
    ...
    def observe(self, symbol: str, timestamp_ms: int, price: float) -> list[dict[str, Any]]:
...
def label_from_series(symbol: str, observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in sorted(observations, key=lambda x: int(x.get("timestamp_ms", 0))):
        output.extend(labeler.observe(symbol, int(row["timestamp_ms"]), float(row["ltp"])))
    return output
