...
def price_action_status() -> dict[str, Any]:
    ...
    results = _results()
    return {"status": "READY" if results else "WAITING", "count": len(results), "scanner": stats, "method": "HISTORICAL_5MIN_MULTI_PATTERN", "research_only": True, "trading": "DISABLED", "note": "Historical 5-minute candles are scanned for range breakouts/breakdowns, H&S, triangles, wedges, flags and rounding structures. A close plus trend/volume confirmation is required before a signal is emitted."}
...
def price_action_results(min_score: float = 65.0) -> dict[str, Any]:
    ...
    stats = price_action_scanner.stats
    return {"count": len(results), "results": results, "source_mode": stats.get("mode"), "cache_date": stats.get("cache_date"), "scanner": stats, "method": "HISTORICAL_5MIN_MULTI_PATTERN", "research_only": True, "trading": "DISABLED", "updated_at": datetime.now(IST).isoformat()}
