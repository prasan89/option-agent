...
def price_action_status() -> dict[str, Any]:
    stats = price_action_scanner.stats
    results = _results()
    return {"status": "READY" if results else "WAITING", "count": len(results), "scanner": stats, "method": "DAILY_PATTERN_15MIN_BREAKOUT", "research_only": True, "trading": "DISABLED", "note": "Historical daily candles are scanned for H&S, triangles, flags, rounding, double/triple tops & bottoms, cup/handle structures, and candlestick reversals. Candlestick reversals include engulfing, harami, piercing, dark cloud cover, morning/evening star, and three soldiers/crows. A 15-minute close plus trend/volume confirmation is required before a signal is emitted."}
...
<div class="top"><div><div class="title">SLO Price Action</div><div class="sub">Daily pattern setup • 15-minute breakout confirmation • research only</div><div class="nav"><a href="/dashboard">Flow Dashboard</a><a href="/strategy">SLO Options</a><a href="/dashboard/history">SLO History</a><a href="/price-action" class="active">Price Action</a><a href="/price-action/history">Price Action History</a><a href="/price-action/paper-pnl">Candlestick P&L</a></div></div><div id="status" class="status">Connecting…</div></div>
...
<div class="panel"><div class="head"><strong>Active Price Action Setups & Confirmed Signals</strong><span id="updated" class="small">—</span></div><div class="sortbar"><label for="sortField">Sort</label><select id="sortField" onchange="setSortField(this.value)"><option value="score">Score</option><option value="time">Time</option><option value="symbol">Symbol</option><option value="signal">Signal</option><option value="pattern">Pattern</option><option value="trigger_level">Trigger</option><option value="close_15min">15M Price</option><option value="vol_ratio_15m">15M Volume</option><option value="ema9_15m">15M EMA9</option><option value="ema20_15m">15M EMA20</option></select><button id="ascBtn" onclick="setSortDir('asc')">↑ ASC</button><button id="descBtn" class="active" onclick="setSortDir('desc')">↓ DESC</button><span id="sortInfo" class="small">Score ↓</span></div><div class="table"><table><thead><tr><th>#</th><th>Status</th><th class="sortable" onclick="sortBy('time')">Time<span class="sortmark" id="mark-time"></span></th><th class="sortable" onclick="sortBy('symbol')">Symbol<span class="sortmark" id="mark-symbol"></span></th><th class="sortable" onclick="sortBy('signal')">Signal<span class="sortmark" id="mark-signal"></span></th><th class="sortable" onclick="sortBy('pattern')">Pattern<span class="sortmark" id="mark-pattern"></span></th><th class="sortable" onclick="sortBy('score')">Score<span class="sortmark" id="mark-score"></span></th><th class="sortable" onclick="sortBy('trigger_level')">Trigger<span class="sortmark" id="mark-trigger_level"></span></th><th class="sortable" onclick="sortBy('close_15min')">15M Price<span class="sortmark" id="mark-close_15min"></span></th><th class="sortable" onclick="sortBy('vol_ratio_15m')">15M Vol<span class="sortmark" id="mark-vol_ratio_15m"></span></th><th class="sortable" onclick="sortBy('ema9_15m')">15M EMA9<span class="sortmark" id="mark-ema9_15m"></span></th><th class="sortable" onclick="sortBy('ema20_15m')">15M EMA20<span class="sortmark" id="mark-ema20_15m"></span></th><th>Reason</th></tr></thead><tbody id="rows"><tr><td colspan="14" class="empty">No active price-action setup or confirmed signal yet.</td></tr></tbody></table></div></div>
<div class="panel"><div class="head"><strong>Scanner Health</strong><span class="small">Groww daily-chart setup + 15-minute candles • no websocket dependency</span></div><div class="metrics"><div class="metric"><span>15-Min Requests</span><b id="intraday">—</b></div><div class="metric"><span>Checks</span><b id="checks">—</b></div><div class="metric"><span>Cached Underlyings</span><b id="cache">—</b></div><div class="metric"><span>Errors</span><b id="errors">—</b></div></div></div>
<div class="panel"><div class="head"><strong>Pattern Engine</strong><span class="small">Heuristic structural detection on daily OHLC setup + 15M confirmation</span></div><div class="metrics"><div class="metric"><span>Classic Reversals</span><b>H&amp;S, Double Top/Bottom, Triple Top/Bottom</b></div><div class="metric"><span>Triangles / Flags</span><b>Triangle and bull/bear flag breakouts</b></div><div class="metric"><span>Rounding / Cup</span><b>Rounding Top/Bottom + Cup &amp; Handle structures</b></div></div></div>
<div class="panel"><div class="head"><strong>Confirmation &amp; Safety</strong><span class="small">Research mode</span></div><div class="metrics"><div class="metric"><span>Score</span><b>Pattern quality + 15M Trend/EMA trend + volume + trigger proximity</b></div><div class="metric"><span>Confirmation</span><b>15-minute candle close through the daily pattern trigger</b></div><div class="metric"><span>History</span><b>Daily pattern history + completed Groww 15-minute candles</b></div><div class="metric"><span>Safety</span><b class="amber">Research only — no orders are placed</b></div></div></div>
</div><script>
const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const fmt=(v,d=2)=>v==null||v===''?'—':Number.isFinite(Number(v))?Number(v).toFixed(d):esc(v);let all=[];let sortField='score';let sortDir='desc';const sortLabels={score:'Score',time:'Time',symbol:'Symbol',signal:'Signal',pattern:'Pattern',trigger_level:'Trigger',close_15min:'15M Price',vol_ratio_15m:'15M Volume',ema9_15m:'15M EMA9',ema20_15m:'15M EMA20'};
