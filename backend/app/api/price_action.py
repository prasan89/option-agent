...
def price_action_status() -> dict[str, Any]:
    ...
    results = _results()
    return {"status": "READY" if results else "WAITING", "count": len(results), "scanner": stats, "method": "HISTORICAL_5MIN_MULTI_PATTERN", "research_only": True, "trading": "DISABLED", "note": "Historical 5-minute candles are scanned for H&S, triangles, falling wedges, flags and rounding structures. A close plus trend/volume confirmation is required before a signal is emitted."}
