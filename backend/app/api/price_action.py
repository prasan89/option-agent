...
def price_action_status() -> dict[str, Any]:
    ...
    results = _results()
    return {"status": "READY" if results else "WAITING", "count": len(results), "scanner": stats, "method": "DAILY_PATTERN_15MIN_BREAKOUT", "research_only": True, "trading": "DISABLED", "note": "Historical daily candles are scanned for H&S, triangles, flags, rounding, double/triple tops & bottoms, and cup/handle structures. A close plus trend/volume confirmation is required before a signal is emitted."}
