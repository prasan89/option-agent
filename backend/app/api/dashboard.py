...
async function refresh(){
 const [sysRes,statusRes,rowsRes]=await Promise.allSettled([fetch(api+'/system/status'),fetch(api+'/signals/status'),fetch(api+'/signals?limit=100')]);
 let sys=null,st=null,rows=[];
 ...
 document.getElementById('status').textContent=db?(st?.running?'● Monitor running':'○ Monitor stopped'):(sys?'● API online • DB unavailable':'Database/API unavailable');
 document.getElementById('count').textContent=st?.persisted_signals??'—';
 document.getElementById('checks').textContent=st?.checks??'—';
 document.getElementById('last').textContent=st?.last_check?new Date(st.last_check).toLocaleTimeString():'—';
 document.getElementById('minscore').textContent=st?.minimum_score!=null?'±'+st.minimum_score:'—';
 ...
 document.getElementById('notice').textContent=db?'Pipeline status is live. Signals are research-only; no orders are placed.':sys?'API is healthy, but PostgreSQL/Redis configuration is required before persisted signals can appear.':'Waiting for the API to become available.';
