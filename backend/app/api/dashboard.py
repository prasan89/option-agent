...
<div class="grid"><div class="card"><div class="label">Persisted Signals</div><div id="count" class="value">—</div></div><div class="card"><div class="label">Monitor Checks</div><div id="checks" class="value">—</div></div><div class="card"><div class="label">F&O Contracts</div><div id="contracts" class="value">—</div></div><div class="card"><div class="label">Underlyings</div><div id="underlyings" class="value">—</div></div><div class="card"><div class="label">Scan Batches</div><div id="batches" class="value">—</div></div></div>
<div id="pipeline" class="pipeline"></div>
<div class="panel"><div class="panelhead"><strong>Scanner Coverage</strong><span class="muted" id="scaninfo">—</span></div><div style="padding:18px 20px" class="coverage"><span>Contracts available: <b id="covavail">—</b></span><span>Contracts scanned: <b id="covscan">—</b></span><span>LTPs received: <b id="covquotes">—</b></span><span>Ranking: <b id="covready">—</b></span></div></div>
<div class="panel"><div class="panelhead"><strong>Top Active Underlyings</strong><span class="muted">After 2 scans • price activity</span></div><div class="tablewrap"><table><thead><tr><th>Rank</th><th>Underlying</th><th>Score</th><th>Direction</th><th>Active Contracts</th><th>Up</th><th>Down</th><th>Max 1m Change</th><th>Top Contract</th></tr></thead><tbody id="underlyingRows"><tr><td colspan="9" class="empty">Waiting for the second scan…</td></tr></tbody></table></div></div>
...
async function refresh(){
 ...
 document.getElementById('batches').textContent=scan?.successful_batches_last_check??'—';
 ...
 document.getElementById('covquotes').textContent=scan?.quotes_received_last_check??'—';
 ...
 document.getElementById('notice').textContent=db?'Full F&O LTP coverage is scanned every minute. Signals are research-only; no orders are placed.':'API is online, but PostgreSQL is unavailable for persistence.';
 const ub=document.getElementById('underlyingRows');
 if(!under.length){ub.innerHTML='<tr><td colspan="9" class="empty">Waiting for the second scan and measurable price movement…</td></tr>'}else{ub.innerHTML=under.map((x,i)=>{const c=x.direction==='UP'?'bull':x.direction==='DOWN'?'bear':'';const t=x.top_contracts?.[0];return `<tr><td>${i+1}</td><td><strong>${esc(x.underlying)}</strong></td><td class="score">${esc(x.activity_score)}</td><td class="${c}">${esc(x.direction)}</td><td>${esc(x.contracts_active)}</td><td>${esc(x.contracts_up)}</td><td>${esc(x.contracts_down)}</td><td>${esc(x.max_change_pct)}%</td><td>${esc(t?.symbol||'—')} ${t?.change_pct==null?'':'('+esc(t.change_pct)+'%)'}</td></tr>`}).join('')}
