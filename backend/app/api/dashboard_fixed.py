...
def dashboard_data() -> dict[str, Any]:
    ...
    return {
        ...
        "opportunities": engine.get("results", [])[:50],
        "strategy": {"count": engine.get("count", 0), "watch_count": engine.get("watch_count", 0), "history_count": engine.get("history_count", 0), "method": engine.get("method", "SLO_OPTIONS_V2_LIVE_ADAPTER"), "diagnostics": engine.get("diagnostics", {}), "research_only": True, "trading": "DISABLED"},
        "signals": signals,
        "signal_count": signal_count,
        "performance": research_store.label_summary(horizon=5),
        "system": {"scanner_running": scan.get("running", False), "feed_contracts": scan.get("symbols_available_last_check", 0), "feed_events": scan.get("feed_events", 0), "ranking_ready": scan.get("ranking_ready", False), "scanner_error": scan.get("last_error") or scan.get("error"), "historical_running": hist.get("running", False), "historical_contracts": hist.get("contracts", 0), "historical_requests": hist.get("requests", 0), "historical_errors": hist.get("errors", 0)},
    }
...
<div class="panel"><div class="head"><b>Today's Research Opportunities</b><span id="method" class="muted"></span></div><div class="table"><table><thead><tr><th>Status</th><th>Underlying</th><th>Option</th><th>Direction</th><th>SLO</th><th>Premium</th><th>Stop</th><th>Target</th><th>DTE</th><th>Volume</th><th>OI</th><th>Reason</th></tr></thead><tbody id="opp"></tbody></table></div></div>
<div class="panel"><div class="head"><b>Active Underlyings</b><span class="muted">Top 25</span></div><div class="table"><table><thead><tr><th>#</th><th>Underlying</th><th>Direction</th><th>Score</th><th>Spot</th><th>Session %</th></tr></thead><tbody id="under"></tbody></table></div></div>
<script>const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const n=(v,d=2)=>v==null?'—':Number.isFinite(Number(v))?Number(v).toFixed(d):esc(v);async function refresh(){try{const r=await fetch('/dashboard/data');if(!r.ok)throw Error(await r.text());const d=await r.json(),m=d.market||{},s=d.strategy||{},sys=d.system||{},reg=d.regime||{},f=d.flow||{},o=d.opportunities||[],u=d.underlyings||[],x=s.diagnostics||{};document.getElementById('badge').textContent=m.label+' • '+new Date(m.now_ist).toLocaleTimeString('en-IN');document.getElementById('notice').textContent=m.state==='LIVE'?'LIVE MARKET: reading Groww feed and research pipeline.': 'Historical/session mode: '+m.label;document.getElementById('mode').textContent=d.source_mode||'—';document.getElementById('session').textContent=m.state;document.getElementById('score').textContent=o.length?n(Math.max(...o.map(z=>Number(z.total_score)||0))):'—';document.getElementById('count').textContent=s.count??0;document.getElementById('watch').textContent=s.watch_count??0;document.getElementById('regime').textContent=reg.state||'WAITING';document.getElementById('regimeScore').textContent='Confidence '+n(reg.confidence,1)+' / signed '+n(reg.signed_score,2);document.getElementById('calls').textContent=(f.call_contracts??0)+' contracts / '+(f.call_activity_pct??'—')+'%';document.getElementById('puts').textContent=(f.put_contracts??0)+' contracts / '+(f.put_activity_pct??'—')+'%';document.getElementById('pcr').textContent=n(f.pcr_oi,3);document.getElementById('method').textContent=s.method||'—';document.getElementById('diagStatus').textContent=(sys.ranking_ready?'RANKING READY':'RANKING NOT READY')+' • feed '+(sys.scanner_running?'RUNNING':'STOPPED');const names={rows:'Rows',option_rows:'Options',premium_rows:'Premium',dte_rows:'DTE',liquid_rows:'Vol/OI',direction_rows:'Direction',score_rows:'Score'};document.getElementById('diag').innerHTML=Object.entries(names).map(([k,v])=>`<div><span class="muted">${v}</span><br><b>${x[k]??0}</b></div>`).join('');document.getElementById('error').textContent=sys.scanner_error?'Scanner error: '+sys.scanner_error:'';document.getElementById('opp').innerHTML=o.length?o.map(z=>`<tr><td class="${z.status==='WATCH'?'warn':z.status==='LIVE'?'good':''}">${esc(z.status)}</td><td><b>${esc(z.underlying)}</b></td><td>${esc(z.option_type)} ${n(z.strike,0)}</td><td>${esc(z.direction)}</td><td>${n(z.total_score)}</td><td>${n(z.premium,4)}</td><td>${n(z.stop_premium,4)}</td><td>${n(z.target_premium,4)}</td><td>${esc(z.dte)}</td><td>${esc(z.volume??'—')}</td><td>${esc(z.open_interest??'—')}</td><td>${esc(z.reason||'—')}</td></tr>`).join(''):'<tr><td colspan="12" class="empty">No current candidate. The diagnostics above show exactly which pipeline stage is empty.</td></tr>';document.getElementById('under').innerHTML=u.length?u.map((z,i)=>`<tr><td>${i+1}</td><td><b>${esc(z.underlying)}</b></td><td>${esc(z.direction||'FLAT')}</td><td>${n(z.activity_score)}</td><td>${n(z.spot,2)}</td><td>${n(z.session_pct,2)}%</td></tr>`).join(''):'<tr><td colspan="6" class="empty">No active underlyings from scanner.</td></tr>'}catch(e){document.getElementById('notice').textContent='Dashboard API error: '+e.message}}refresh();setInterval(refresh,10000);</script></div></body></html>'''
