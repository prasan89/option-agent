...
class ResearchPipeline:
    ...
    @property
    def stats(self):
        try: research_store.init();db="CONNECTED"
        except Exception: db="NOT_CONNECTED"
        return {"running":self.running,"auto_start":settings.auto_start_pipeline,"feed":feed_service.running,"feed_instruments":len(feed_service.instruments),"flow":flow_engine.running,
            "scanner":fno_scanner.running,"intelligence_score":intelligence_score_engine.running,"dataset":historical_dataset_collector.running,"ml":ml_engine.running,
            "signal_monitor":signal_monitor.running,"feed_symbols_selected":self._feed_symbols,"last_error":self._last_error,"event_bus":research_event_bus.stats,"database":db,"trading":"DISABLED"}
    @staticmethod
    def _feed_instruments():
        ...
        nearest=sorted({str(r.get("expiry_date"))[:10] for r in eligible})[0];selected=[r for r in eligible if str(r.get("expiry_date"))[:10]==nearest]
    ...
    def start(self):
        with self._lock:
            ...
            except Exception:signal_monitor.stop();raise
    def stop(self):
        with self._lock:
